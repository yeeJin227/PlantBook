...
public class Gangnangkong_Four : MonoBehaviour
{
    ...
    public GameObject supportStick; //지지대 오브젝트
    public GameObject fixedStick; // 활성화할 고정된 지지대
    public GameObject fixedGangnangkong; //곧은 강낭콩
    ...
    public float supportDistance = 1.0f; //지지대와 고정할 위치 사이의 거리
    ...
    void Update()
    {
        ...
        if (distance <= activationDistance)
        {
            ...
            if(distance_support <= supportDistance){
                supportStick.SetActive(false); // 오브젝트 비활성화
                fixedStick.SetActive(true);
                Debug.Log("지지대 비활성화됨");
                currentObject.SetActive(false);
                fixedGangnangkong.SetActive(true);
            }
